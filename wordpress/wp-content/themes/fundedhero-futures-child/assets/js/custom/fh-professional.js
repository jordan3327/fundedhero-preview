...
(function () {
  ...
  function moveAfter(reference, element) {
    if (reference && element && reference !== element) reference.insertAdjacentElement('afterend', element);
  ...
  function arrangeHomepage() {
    ...
    cursor = moveAfter(cursor, document.querySelector('.fh-steps'));
    cursor = moveAfter(cursor, document.querySelector('.fh-hero'));
    moveAfter(cursor, document.querySelector('.fh-community'));
    pricing.dataset.fhPositioned = 'true';
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', arrangeHomepage, { once: true });
  else arrangeHomepage();
})();
